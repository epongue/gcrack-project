...
class GCrackData(GCrackBase):
    def generate_mesh(self, crack_points: List[np.ndarray]) -> gmsh.model:
        ...
        # Mid
        p3: int = gmsh.model.geo.addPoint(-xc - r, 0, 0, h)  # Mid right node
        p4: int = gmsh.model.geo.addPoint(xc + r, 0, 0, h)  # Mid left node
        ...
        #centers
        c1: int = gmsh.model.geo.addPoint(xc, 0, 0, h)
        c2: int = gmsh.model.geo.addPoint(-xc, 0, 0, h)
        ...
        # Lines
        # Bot
        lb1: int = -gmsh.model.geo.addCircleArc(p3, c2, p1)
        ...
        lb4: int = -gmsh.model.geo.addCircleArc(p2, c1, p4)
        lb5: int = gmsh.model.geo.addLine(p2, p1)
        # Top
        lt1: int = -gmsh.model.geo.addCircleArc(p3, c2, p5)
        ...
        lt4: int = -gmsh.model.geo.addCircleArc(p6, c1, p4)
...
if __name__ == "__main__":
    ...
    for name, case in cases.items():
        ...
        pars = {}
        pars["L"] = case["L"]  # TODO: Set the real length (per case) !!!
        pars["W"] = case["W"]  # TODO: Set the real width (per case) !!!
        pars["a0"] = case["a0"]
        pars["Gc"] = 380
