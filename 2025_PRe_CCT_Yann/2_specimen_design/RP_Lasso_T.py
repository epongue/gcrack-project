...
for I, (i,j) in enumerate(itertools.product(range(M_angle), range(N_rayon))):
    ...
    fichier_csv = f'/home/epongue/Documents/gcrack-main/lefm_gcrack_script/E2_s=0/angle_{angle[i]}/results_{rayon[j]}_{angle[i]}.csv'
...
k = np.where(KI_target >= 0)[0] #indices des composantes de KI_target positives 

# #verification de la cohérence des données
# fichier_csv = f'/home/epongue/Documents/gcrack-main/lefm_gcrack_script/E2_s=0/angle_{angle[0]}/results_{rayon[6]}_{angle[0]}.csv'
...
# T = df1.iloc[1, 13]*np.sqrt(np.pi*a0[0]) / (Keq)

# print(T_target[6]-T) # doit être égale à 0
...
#creation of the folder where the data will be stored
os.makedirs(f'ResultsRegression/coef_poly_T_N={N_degree}', exist_ok=True)

#data
angle_input = angle_input[k]
angle_data1 = np.cos(angle_input)
angle_data2 = np.tan(angle_input)
dW_data = dW_input[k]
X = np.column_stack((angle_data1, angle_data2, dW_data))
y = T_target[k] # T
...
X_train, X_test, y_train, y_true = train_test_split(X_poly_scaled, y, test_size=0.2, random_state=42) # for T
...
for i in range(len(beta)):
    ...
    # chosen beta
    print(f"\n Paramètre beta pour la prédiction de T: {beta[i]}") 
    ...
    feature_names = poly.get_feature_names_out(input_features=[r'\cos(\alpha)', r'\tan(\alpha)', r'\Delta W'])

    # Coefficients DataFrame for T
    ...
    print(f"\nScore R² pour la prédiction de T : {r2_score_test:.3f}")
    ...
    e = np.linalg.norm(y_pred - y_true, ord=2) / np.linalg.norm(y_true, ord=2)  # RRMSE

    # performance evaluation
    print(f"\n RRMSE pour la prédiction de T : {e:.3f}"+" \n--------FIN--------")

    # add results to the table
    results_table.loc[i] = [beta[i], r2_score_test, non_zero_coef, e]

    # Save coefficients to CSV
    coefficients_df.to_csv(f'ResultsRegression/coef_poly_T_N={N_degree}/coefs_T_beta={beta[i]:.5f}_r2={r2_score_test:.3f} .csv', index=False)

        # # -------------------- Affichage ------------------------#
    y_pred = model.predict(X_test)


    # # model verification for T prediction for a given beta
    ...
    # ax.set_xlabel("T_test adim", fontsize = 16)
    # ax.set_ylabel("T_prédit adim", fontsize = 16)
    # ax.axis('equal')
...
# save results to CSV
results_table.to_csv(f"ResultsRegression/choix_beta_T_N={N_degree}.csv", index=False)
